...
describe("GOLD", function() {
    ...
    describe("pause()", function () {
        ...
        it("should pause contract correctly", async function() {
            ...
            await expect(token.transfer(accountB.address, amount)).to.be.revertedWith("Pausable:")
        })
    })
    describe("unpause()", function () {
        ...
        it("should rever if contract has been unpause", async function() {
            await token.unpause();
        ...
        it("should pause contract correctly", async function() {
            ...
            await expect(transferTx).to.emit(token, "Transfer").withArgs(accountA.address, accountB.address)
        })
    })

    describe("addToBlacklist()", function() {
        it("should revert in case add sender to blacklist", async function() {
            await expect(token.addToBlacklist(accountA.address)).to.be.revertedWith("Gold: can not add sender to blacklist")
        })
        it("should revert if account in on blacklist", async function() {
            await token.addToBlacklist(accountA.address);
            await expect(token.addToBlacklist(accountA.address).to.be.revertedWith("Gold: account has been add to blacklist"));
        })
        it("should revert if not admin role", async function() {
            await expect(token.connect(accountB.address).addToBlacklist(accountC.address)).to.be.revertedWith("Gold: current acount is not Admin");
        })
